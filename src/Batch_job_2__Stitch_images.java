...
public class Batch_job_2__Stitch_images implements PlugIn {
	...
	public void run(String arg) {
		...
		for(int i=0; i<arrAngles.size(); i++) {
			...
			gd1.addNumericField("Last index of "+ String.valueOf(arrAngles.get(i)) + " degree: ", (i+1) * NumImg - 1,0);
		}
		gd1.showDialog();
		...
		for(int i=0; i<arrAngles.size(); i++) {
			...
			if(arrStartIdx[i] < 0 || arrStartIdx[i] >= arrEndIdx[i] || Math.round(arrStartIdx[i]) != arrStartIdx[i] || Math.round(arrEndIdx[i]) != arrEndIdx[i]){
		...
		for(int i=0; i<NumImg; i++) {
			...
			for(int j=0; j<arrAngles.size(); j++) {
				...
				if(j==0) {
					...
					if(arrMin2q[j] < globalMin2q)
						globalMin2q = arrMin2q[j];
					if(arrMax2q[j] > globalMax2q)
						globalMax2q = arrMax2q[j];
				}
			}
			...
			for(int j=0; j<w_new; j++) {
				int idxToUse = 0;
				if(arrAngles.size() > 1) {
					while(globalMax2q - globalStep2q * j > arrMin2q[idxToUse + 1]) {
						idxToUse++;
		...
		try {
			...
			for(int i=0; i<arrAngles.size(); i++) {
				pw.println("File index " + String.valueOf(arrStartIdx[i]) + "-" + String.valueOf(arrEndIdx[i]) + " for camera angle " + String.valueOf(arrAngles.get(i))+ " degree.");
			}
